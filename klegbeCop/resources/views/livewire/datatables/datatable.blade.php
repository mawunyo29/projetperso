<div>
    ...
    @endif
    <div class="relative shadow-lg border-collapse border-gray-50 ">
        <div class="md:flex block justify-between items-center mb-1 bg-pink-600 p-2">
            <div class="flex-grow h-10 flex items-center rounded-md my-4">
                @if($this->searchableColumns()->count())
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10 rounded-md">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none ">
                            <svg class="h-5 w-5 text-gray-600" viewBox="0 0 20 20" stroke="currentColor" fill="none">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                        </div>
                        <input wire:model.debounce.500ms="search"  class="form-input block   focus:bg-transparent  focus:text-white w-full h-10 ring-0   items-center p-1    hover:text-gray-900 focus:outline-none focus:shadow-outline transform hover:scale-105 duration-300 ease-in-out  rounded-md pl-10 transition   sm:text-sm sm:leading-5" placeholder="Search in {{ $this->searchableColumns()->map->label->join(', ') }}" />
        ...
        @if($hideable === 'buttons')
        <div class="p-2 grid grid-cols-8 gap-2 text-justify">
        ...
        <div class="rounded-lg shadow-lg bg-white max-w-screen ">
            <div class="overflow-x-scroll lg:overflow-x-hidden rounded-lg @unless($this->hidePagination) rounded-b-none @endif">
                ...
                <div class="p-2 sm:flex items-center justify-between">
                    ...
                    @if($this->results[1])
                        <div class="my-2 sm:my-0 flex items-center ">
                            <select name="perPage" class="mt-1 form-select rounded-md block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
